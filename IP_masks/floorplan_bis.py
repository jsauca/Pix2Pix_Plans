...
def mergeLines(line_1, line_2):
    ...
    if direction_1 == 0:
        return [(min(min_1, min_2), fixedValue), (max(max_1,
                                                      max_2), fixedValue)]
    else:
        return [(fixedValue, min(min_1, min_2)), (fixedValue, max(max_1,
                                                                  max_2))]
    return
...
def findConnections(line_1, line_2, gap):
    ...
    for c_1 in range(2):
        ...
        for c_2 in range(2):
            ...
            connection_1 = c_1
            connection_2 = c_2
            connectionPoint = ((line_1[c_1][0] + line_2[c_2][0]) // 2,
                               (line_1[c_1][1] + line_2[c_2][1]) // 2)
            pointConnected = True
            break
        continue
    if pointConnected:
    ...
    for lineIndex_1, line_1 in enumerate(lines):
        for lineIndex_2, line_2 in enumerate(lines):
            if lineIndex_2 == lineIndex_1:
                continue
            connections, connectionPoint = findConnections(line_1,
                                                           line_2,
                                                           gap=gap)
            ...
            for c in range(2):
                ...
                continue
            corners.append(
                (connectionPoint, connectionCornerMap[tuple(connections)]))
            continue
        continue
...
def augmentSample(options, image, background_colors=[], split='train'):
    max_size = np.random.randint(low=int(options.width * 3 / 4),
                                 high=options.width + 1)
    if split != 'train':
        max_size = options.width
        pass
    image_sizes = np.array(image.shape[:2]).astype(np.float32)
    transformation = np.zeros((3, 3))
    transformation[0][0] = transformation[1][1] = float(
        max_size) / image_sizes.max()
    ...
    if len(background_colors) == 0:
        ...
        full_image = background_colors[np.random.choice(
            ...
            options.width * options.height)].reshape(
...
def getitem(path, info_width=512, info_height=512):
    ...
    with open(path) as info_file:
        ...
        for line in info_file:
            ...
            if label == 'wall':
                walls.append(
                    (convertToPoint(line[0],
                                    line[1]), convertToPoint(line[2], line[3])))
    ...
    for wall_index_1, (wall_1, wall_type_1) in enumerate(zip(walls,
                                                             wall_types)):
        for wall_index_2, (wall_2,
                           wall_type_2) in enumerate(zip(walls, wall_types)):
            if wall_type_1 == 0 and wall_type_2 == 1 and calcLineDirection(
                    wall_1) == calcLineDirection(wall_2):
                if min([
                        pointDistance(wall_1[c_1], wall_2[c_2])
                        for c_1, c_2 in [(0, 0), (0, 1), (1, 0), (1, 1)]
                ]) <= gap * 2:
        ...
        continue
    walls = [
        wall for wall_index, wall in enumerate(walls)
        if wall_index not in invalid_indices
    ]

    background_mask = measure.label(
        1 - drawWallMask(walls, image_width, image_height), background=0)
    ...
    #                                           background_colors)
    # else:
    #     image, transformation = augmentSample(self.options,
    #                                           image,
    #                                           background_colors,
    #                                           split=self.split)
    #     pass

    # corners = [(transformPoint(transformation, corner[0]), corner[1])
    #            for corner in corners]
    # walls = [[transformPoint(transformation, wall[c])
    #           for c in range(2)]
    #          for wall in walls]
    # doors = [[transformPoint(transformation, door[c])
    #           for c in range(2)]
    #          for door in doors]
    ...
    if debug >= 0:
        cv2.imwrite('test/floorplan/rooms.png',
                    drawSegmentationImage(rooms, blackIndex=backgroundIndex))
        exit(1)
        pass
    ...
    for corner in corner_gt:
        cornerSegmentation[min(max(corner[1], 0), height - 1),
                           min(max(corner[0], 0), width - 1), corner[2] - 1] = 1
        continue
    ...
    if False:
        ...
        print([
            (seg.min(), seg.max(), seg.shape)
            for seg in [cornerSegmentation, iconSegmentation, roomSegmentation]
        ])
        exit(1)
        pass
    ...
    sample = [
        cornerSegmentation.astype(np.float32),
        iconSegmentation.astype(np.int64),
