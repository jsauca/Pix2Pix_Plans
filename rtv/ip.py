...
def extractCorners(heatmaps,
            ...
            if pointType in [2]:
                ...
                for i in range(len(orientations)):
                    ...
                    for corner in corners:
                        orientationPoints[(pointType - 1) * 4 +
                                          newOrientation].append(corner +
                                                                 (True,))
                        continue
                    continue
            elif pointType in [1]:
    ...
    for orientationIndex, corners in enumerate(orientationPoints):
        ...
        if cornerType == 'wall':
            points += [[
                corner[0][0], corner[0][1], orientationIndex // 4,
                orientationIndex % 4
            ] for corner in corners]
        elif cornerType == 'door':
            points += [[corner[0][0], corner[0][1], 0, orientationIndex]
                       for corner in corners]
        else:
            points += [[corner[0][0], corner[0][1], 1, orientationIndex]
                       for corner in corners]
            pass
        continue
    ...
    for orientationIndex, corners in enumerate(orientationPoints):
        ...
        for orientation in orientations:
            ...
            for cornerIndex, corner in enumerate(corners):
                ...
                ranges[lineDim + 2] = max(ranges[lineDim + 2],
                                          corner[0][lineDim])
                ranges[1 - lineDim] = min(ranges[1 - lineDim],
                                          corner[1][1 - lineDim] - gap)
                ranges[1 - lineDim + 2] = max(ranges[1 - lineDim + 2],
                                              corner[2][1 - lineDim] + gap)

                for oppositeOrientationIndex, oppositeCorners in enumerate(
                        orientationPoints):
                    if oppositeOrientation not in cornerOrientations[
                            oppositeOrientationIndex]:
                        continue
                    for oppositeCornerIndex, oppositeCorner in enumerate(
                            oppositeCorners):
                        if orientationIndex == oppositeOrientationIndex and oppositeCornerIndex == cornerIndex:
                            continue

                        oppositePointIndex = pointOffsets[
                            oppositeOrientationIndex] + oppositeCornerIndex

                        if oppositeCorner[0][lineDim] < ranges[
                                lineDim] or oppositeCorner[0][lineDim] > ranges[
                                    lineDim +
                                    2] or ranges[1 - lineDim] > oppositeCorner[
                                        2][1 - lineDim] or ranges[
                                            1 - lineDim +
                                            2] < oppositeCorner[1][1 - lineDim]:
                            continue

                        if abs(oppositeCorner[0][lineDim] - corner[0][lineDim]
                              ) < LENGTH_THRESHOLDS[cornerType]:
                            continue

                        lineIndex = len(lines)
                        pointOrientationLinesMap[pointIndex][
                            orientation].append(lineIndex)
                        pointOrientationLinesMap[oppositePointIndex][
                            oppositeOrientation].append(lineIndex)
    ...
    for pointIndex, point in enumerate(points):
        ...
        for orientation in range(4):
            ...
            for neighborPointIndex, neighborPoint in enumerate(points):
                ...
                neighborOrientations = POINT_ORIENTATIONS[neighborPoint[2]][
                    neighborPoint[3]]
                ...
                inRange = True
                for c in range(2):
                    if neighborPoint[c] < ranges[c] or neighborPoint[
                            c] > ranges[c + 2]:
                        inRange = False
                        break
                    continue

                if not inRange or abs(neighborPoint[lineDim] -
                                      point[lineDim]) < max(
                                          abs(neighborPoint[1 - lineDim] -
                                              point[1 - lineDim]), 1):
                    continue
...
def filterWalls(wallPoints, wallLines):
    ...
    while True:
        ...
        for line in wallLines:
            ...
            for c, pointIndex in enumerate(line):
                ...
                if orientation not in pointOrientationNeighborsMap[pointIndex]:
                    ...
                    pass
                pointOrientationNeighborsMap[pointIndex][orientation].append(
                    line[1 - c])
                continue
            continue
        ...
        for pointIndex, point in enumerate(wallPoints):
            ...
            if len(orientationNeighborMap) < len(orientations):
                if len(orientationNeighborMap) >= 2 and tuple(
                        orientationNeighborMap.keys()) in orientationMap:
                    newOrientation = orientationMap[tuple(
                        orientationNeighborMap.keys())]
    ...
    for lineIndex, line in enumerate(wallLines):
        ...
        for c, pointIndex in enumerate(line):
            if lineDim == 0:
                if wallPoints[pointIndex][lineDim] < wallPoints[line[
                        1 - c]][lineDim]:
                    orientation = 1
                else:
                    orientation = 3
...
def writeDoors(points, lines, doorTypes, output_prefix='test/'):
    with open(output_prefix + 'doors_out.txt', 'w') as doors_file:
        for lineIndex, line in enumerate(lines):
            ...
            doors_file.write(
                str(point_1[0] + 1) + '\t' + str(point_1[1] + 1) + '\t')
            doors_file.write(
                str(point_2[0] + 1) + '\t' + str(point_2[1] + 1) + '\t')
...
def adjustPoints(points, lines):
    ...
    for lineIndex, line in enumerate(lines):
        ...
        for neighborLineIndex, neighborLine in enumerate(lines):
            ...
            if lineDimNeighbor != lineDim:
                continue
            if neighborLine[0] != line[0] and neighborLine[0] != line[
                    1] and neighborLine[1] != line[0] and neighborLine[
                        1] != line[1]:
                continue
    ...
    for lineIndex in range(len(lines)):
        ...
        xy = np.concatenate([
            np.array(
                [points[pointIndex][:2]
                 for pointIndex in lines[lineIndex]])
            for lineIndex in lineGroup
        ],
                            axis=0)
        mins = xy.min(0)
        maxs = xy.max(0)
    ...
    for pointIndex_1, point_1 in enumerate(points):
        ...
        for pointIndex_2, point_2 in enumerate(points):
            ...
            if pointDistance(point_1[:2], point_2[:2]) <= DISTANCES['point']:
                orientations = list(POINT_ORIENTATIONS[point_1[2]][point_1[3]] +
                                    POINT_ORIENTATIONS[point_2[2]][point_2[3]])
                if len([
                        line for line in lines
                        if pointIndex_1 in line and pointIndex_2 in line
                ]) > 0:
                    if abs(point_1[0] - point_2[0]) > abs(point_1[1] -
                                                          point_2[1]):
                ...
                if orientations not in orientationMap:
                    ...
                    lineIndices_1 = [
                        (lineIndex, tuple(set(line) - set((pointIndex_1,)))[0])
                        for lineIndex, line in enumerate(lines)
                        if pointIndex_1 in line and pointIndex_2 not in line
                    ]
                    lineIndices_2 = [
                        (lineIndex, tuple(set(line) - set((pointIndex_2,)))[0])
                        for lineIndex, line in enumerate(lines)
                        if pointIndex_2 in line and pointIndex_1 not in line
                    ]
                ...
                newPoint = [(point_1[0] + point_2[0]) // 2,
                            (point_1[1] + point_2[1]) // 2, pointInfo[0],
                            pointInfo[1]]
    ...
    for pointIndex, orientationNeighborMap in pointOrientationNeighborsMap.items(
        ...
        for neighborIndex_1 in orientationNeighborMap[1]:
            if 2 not in pointOrientationNeighborsMap[neighborIndex_1]:
                continue
            lastCornerCandiates = pointOrientationNeighborsMap[neighborIndex_1][
                2]
            for neighborIndex_2 in orientationNeighborMap[2]:
                if 1 not in pointOrientationNeighborsMap[neighborIndex_2]:
                    continue
                for lastCornerIndex in pointOrientationNeighborsMap[
                        neighborIndex_2][1]:
                    ...
                    #continue
                    if (x_2 - x_1 + 1) * (y_2 - y_1 + 1) <= LENGTH_THRESHOLDS[
                            'icon'] * LENGTH_THRESHOLDS['icon']:
                        continue

                    icons.append((pointIndex, neighborIndex_1, neighborIndex_2,
                                  lastCornerIndex))
                    continue
                continue
            continue
...
def findLineNeighbors(points, lines, labelVotesMap, gap):
    ...
    for lineIndex, line in enumerate(lines):
        ...
        for neighborLineIndex, neighborLine in enumerate(lines):
            ...
            minValue = max(points[line[0]][lineDim],
                           points[neighborLine[0]][lineDim])
            maxValue = min(points[line[1]][lineDim],
                           points[neighborLine[1]][lineDim])
        ...
        for lineIndex, neighbors in enumerate(lineNeighbors):
            ...
            for neighbor_1, region_1 in neighbors[1].items():
                for neighbor_2, _ in neighbors[0].items():
                    ...
                    if region_1[0][
                            lineDim] < region_2[0][lineDim] + gap and region_1[
                                1][lineDim] > region_2[1][lineDim] - gap:
    ...
    for lineIndex, directionNeighbors in enumerate(lineNeighbors):
        for direction, neighbors in enumerate(directionNeighbors):
            for neighbor, region in neighbors.items():
                labelVotes = labelVotesMap[:, region[1][1], region[1][
                    0]] + labelVotesMap[:, region[0][1], region[0][
                        0]] - labelVotesMap[:, region[0][1], region[1][
                            0]] - labelVotesMap[:, region[1][1], region[0][0]]
...
def findRectangleLineNeighbors(rectanglePoints, rectangles, linePoints, lines,
                               lineNeighbors, gap, distanceThreshold):
    ...
    for rectangleIndex, rectangle in enumerate(rectangles):
        for lineIndex, line in enumerate(lines):
            ...
            minValue = max(rectanglePoints[rectangle[0]][lineDim],
                           rectanglePoints[rectangle[2 - lineDim]][lineDim],
                           linePoints[line[0]][lineDim])
            maxValue = min(rectanglePoints[rectangle[1 + lineDim]][lineDim],
                           rectanglePoints[rectangle[3]][lineDim],
                           linePoints[line[1]][lineDim])

            if maxValue - minValue < gap:
                continue

            rectangleFixedValue_1 = (
                rectanglePoints[rectangle[0]][1 - lineDim] +
                rectanglePoints[rectangle[1 + lineDim]][1 - lineDim]) // 2
            rectangleFixedValue_2 = (
                rectanglePoints[rectangle[2 - lineDim]][1 - lineDim] +
                rectanglePoints[rectangle[3]][1 - lineDim]) // 2
            lineFixedValue = (linePoints[line[0]][1 - lineDim] +
                              linePoints[line[1]][1 - lineDim]) // 2
            ...
            if lineFixedValue <= rectangleFixedValue_1 + gap:
                ...
                if index not in minDistanceLineNeighbors or distance < minDistanceLineNeighbors[
                        index][1]:
                    minDistanceLineNeighbors[index] = (lineIndex, distance,
                                                       1 - lineDim)
            else:
                index = lineDim * 2 + 1
        ...
        if len(rectangleLineNeighbors[rectangleIndex]) == 0 or True:
            for index, lineNeighbor in minDistanceLineNeighbors.items():
                rectangleLineNeighbors[rectangleIndex][
                    lineNeighbor[0]] = lineNeighbor[2]
                continue
            pass
        continue
...
def findLineMap(points, lines, points_2, lines_2, gap):
    ...
    for lineIndex, line in enumerate(lines):
        ...
        for neighborLineIndex, neighborLine in enumerate(lines_2):
            ...
            minValue = max(points[line[0]][lineDim],
                           points_2[neighborLine[0]][lineDim])
            maxValue = min(points[line[1]][lineDim],
                           points_2[neighborLine[1]][lineDim])
            if maxValue - minValue < gap:
                continue
            fixedValue_1 = (points[line[0]][1 - lineDim] +
                            points[line[1]][1 - lineDim]) // 2
            fixedValue_2 = (points_2[neighborLine[0]][1 - lineDim] +
                            points_2[neighborLine[1]][1 - lineDim]) // 2
            ...
            ratio = float(maxValue - minValue + 1) / (lineMaxValue -
                                                      lineMinValue + 1)
...
def findConflictLinePairs(points,
    ...
    for lineIndex_1, line_1 in enumerate(lines):
        ...
        fixedValue_1 = int(
            round((point_1[1 - lineDim_1] + point_2[1 - lineDim_1]) // 2))
        ...
        for lineIndex_2, line_2 in enumerate(lines):
            ...
            if considerEndPoints:
                if min([
                        pointDistance(points[line_1[0]], points[line_2[0]]),
                        pointDistance(points[line_1[0]], points[line_2[1]]),
                        pointDistance(points[line_1[1]], points[line_2[0]]),
                        pointDistance(points[line_1[1]], points[line_2[1]])
                ]) <= gap:
                    conflictLinePairs.append((lineIndex_1, lineIndex_2))
                    continue
                pass
            ...
            fixedValue_2 = int(
                round((point_1[1 - lineDim_2] + point_2[1 - lineDim_2]) // 2))
            ...
            if lineDim_1 == lineDim_2:
                if abs(
                        fixedValue_2 - fixedValue_1
                ) >= distanceThreshold or minValue_1 > maxValue_2 - gap or minValue_2 > maxValue_1 - gap:
                    continue
        ...
        for rectangleIndex_2, rectangle_2 in enumerate(rectangles):
            ...
            for cornerIndex in range(4):
                if rectangle_1[cornerIndex] == rectangle_2[cornerIndex]:
                    conflictRectanglePairs.append(
                        (rectangleIndex_1, rectangleIndex_2))
                    conflict = True
                    break
                continue
            ...
            minX = max(
                (points[rectangle_1[0]][0] + points[rectangle_1[2]][0]) // 2,
                (points[rectangle_2[0]][0] + points[rectangle_2[2]][0]) // 2)
            maxX = min(
                (points[rectangle_1[1]][0] + points[rectangle_1[3]][0]) // 2,
                (points[rectangle_2[1]][0] + points[rectangle_2[3]][0]) // 2)
            if minX > maxX - gap:
                continue
            minY = max(
                (points[rectangle_1[0]][1] + points[rectangle_1[1]][1]) // 2,
                (points[rectangle_2[0]][1] + points[rectangle_2[1]][1]) // 2)
            maxY = min(
                (points[rectangle_1[2]][1] + points[rectangle_1[3]][1]) // 2,
                (points[rectangle_2[2]][1] + points[rectangle_2[3]][1]) // 2)
            if minY > maxY - gap:
                continue
...
def findConflictRectangleLinePairs(rectanglePoints, rectangles, linePoints,
                                   lines, gap):
    ...
    for rectangleIndex, rectangle in enumerate(rectangles):
        for lineIndex, line in enumerate(lines):
            lineDim = calcLineDim(linePoints, line)
            if lineDim == 0:
                minX = max(rectanglePoints[rectangle[0]][0],
                           rectanglePoints[rectangle[2]][0],
                           linePoints[line[0]][0])
                maxX = min(rectanglePoints[rectangle[1]][0],
                           rectanglePoints[rectangle[3]][0],
                           linePoints[line[1]][0])
                if minX > maxX - gap:
                    continue
                if max(rectanglePoints[rectangle[0]][1],
                       rectanglePoints[rectangle[1]][1]) + gap > min(
                           linePoints[line[0]][1], linePoints[line[1]][1]):
                    continue
                if min(rectanglePoints[rectangle[2]][1],
                       rectanglePoints[rectangle[3]][1]) - gap < max(
                           linePoints[line[0]][1], linePoints[line[1]][1]):
                    continue

            elif lineDim == 1:
                minY = max(rectanglePoints[rectangle[0]][1],
                           rectanglePoints[rectangle[1]][1],
                           linePoints[line[0]][1])
                maxY = min(rectanglePoints[rectangle[2]][1],
                           rectanglePoints[rectangle[3]][1],
                           linePoints[line[1]][1])
                if minY > maxY - gap:
                    continue
                if max(rectanglePoints[rectangle[0]][0],
                       rectanglePoints[rectangle[2]][0]) + gap > min(
                           linePoints[line[0]][0], linePoints[line[1]][0]):
                    continue
                if min(rectanglePoints[rectangle[1]][0],
                       rectanglePoints[rectangle[3]][0]) - gap < max(
                           linePoints[line[0]][0], linePoints[line[1]][0]):
                    continue
...
def findLinePointMap(points, lines, points_2, gap):
    ...
    for lineIndex, line in enumerate(lines):
        lineDim = calcLineDim(points, line)
        fixedValue = (points[line[0]][1 - lineDim] +
                      points[line[1]][1 - lineDim]) // 2
        for neighborPointIndex, neighborPoint in enumerate(points_2):
            if neighborPoint[lineDim] < points[
                    line[0]][lineDim] + gap or neighborPoint[lineDim] > points[
                        line[1]][lineDim] - gap:
                continue

            if abs((neighborPoint[1 - lineDim] + neighborPoint[1 - lineDim]) //
                   2 - fixedValue) > gap:
                continue
...
def findCandidatesFromHeatmaps(iconHeatmaps, iconPointOffset, doorPointOffset):
    ...
    for iconIndex in range(1, NUM_ICONS + 2):
        ...
        heatmap = cv2.dilate(cv2.erode(heatmap.astype(np.uint8), kernel),
                             kernel)
        ...
        for regionIndex in range(regions.min() + 1, regions.max() + 1):
            ...
            if iconIndex <= NUM_ICONS:
                if maxX - minX < GAPS['icon_extraction'] or maxY - minY < GAPS[
                        'icon_extraction']:
                    continue
                ...
                for y in range(sizeY - 1, -1, -1):
                    ...
                    maxY -= 1
                    continue
                if (maxY - minY + 1) * (maxX - minX + 1) <= LENGTH_THRESHOLDS[
                        'icon'] * LENGTH_THRESHOLDS['icon'] * 2:
                    continue
                newIconPoints += [[minX, minY, 1, 2], [maxX, minY, 1, 3],
                                  [minX, maxY, 1, 1], [maxX, maxY, 1, 0]]
                newIcons.append((iconPointOffset, iconPointOffset + 1,
                                 iconPointOffset + 2, iconPointOffset + 3))
                iconPointOffset += 4
            else:
                sizeX, sizeY = maxX - minX + 1, maxY - minY + 1
                if sizeX >= LENGTH_THRESHOLDS['door'] and sizeY * 2 <= sizeX:
                    newDoorPoints += [[minX, (minY + maxY) // 2, 0, 1],
                                      [maxX, (minY + maxY) // 2, 0, 3]]
                    newDoorLines.append((doorPointOffset, doorPointOffset + 1))
                    doorPointOffset += 2
                elif sizeY >= LENGTH_THRESHOLDS['door'] and sizeX * 2 <= sizeY:
                    newDoorPoints += [[(minX + maxX) // 2, minY, 0, 2],
                                      [(minX + maxX) // 2, maxY, 0, 0]]
                    newDoorLines.append((doorPointOffset, doorPointOffset + 1))
                    doorPointOffset += 2
                elif sizeX >= LENGTH_THRESHOLDS[
                        'door'] and sizeY >= LENGTH_THRESHOLDS['door']:
                    mask = regionMask[minY:maxY + 1, minX:maxX + 1]
                    sumX = mask.sum(0)
                    ...
                    if (sizeX - minOffset - maxOffset
                       ) * 2 <= sizeY and sizeX - minOffset - maxOffset > 0:
                        newDoorPoints += [
                            [(minX + minOffset + maxX - maxOffset) // 2, minY,
                             0, 2],
                            [(minX + minOffset + maxX - maxOffset) // 2, maxY,
                             0, 0]
                        ]
                        newDoorLines.append(
                            (doorPointOffset, doorPointOffset + 1))
                        doorPointOffset += 2
                        pass
                    ...
                    if (sizeY - minOffset - maxOffset
                       ) * 2 <= sizeX and sizeY - minOffset - maxOffset > 0:
                        newDoorPoints += [
                            [
                                minX,
                                (minY + minOffset + maxY - maxOffset) // 2, 0, 1
                            ],
                            [
                                maxX,
                                (minY + minOffset + maxY - maxOffset) // 2, 0, 3
                            ]
                        ]
                        newDoorLines.append(
                            (doorPointOffset, doorPointOffset + 1))
...
def reconstructFloorplan(wallCornerHeatmaps,
    ...
    wallPoints, wallLines, wallPointOrientationLinesMap, wallPointNeighbors, augmentedPointMask = extractCorners(
        wallCornerHeatmaps,
        heatmapValueThresholdWall,
        gap=GAPS['wall_extraction'],
        augment=enableAugmentation,
        gt=gt)
    doorPoints, doorLines, doorPointOrientationLinesMap, doorPointNeighbors, _ = extractCorners(
        doorCornerHeatmaps,
        heatmapValueThresholdDoor,
        gap=GAPS['door_extraction'],
        cornerType='door',
        gt=gt)
    iconPoints, iconLines, iconPointOrientationLinesMap, iconPointNeighbors, _ = extractCorners(
        iconCornerHeatmaps,
        heatmapValueThresholdIcon,
        gap=GAPS['icon_extraction'],
        cornerType='icon',
        gt=gt)
    ...
    if not gt:
        ...
        wallPoints, wallLines, wallPointOrientationLinesMap, wallPointNeighbors = filterWalls(
            wallPoints, wallLines)
        pass

    sortLines(doorPoints, doorLines)
    sortLines(wallPoints, wallLines)
    if do_print:
        print('the number of points', len(wallPoints), len(doorPoints),
              len(iconPoints))
        print('the number of lines', len(wallLines), len(doorLines),
              len(iconLines))

    drawPoints(os.path.join(debug_prefix, "points.png"),
               width,
               height,
               wallPoints,
               densityImage,
               pointSize=3)
    drawPointsSeparately(os.path.join(debug_prefix, 'points'),
                         width,
                         height,
                         wallPoints,
                         densityImage,
                         pointSize=3)
    drawLines(os.path.join(debug_prefix, 'lines.png'),
              width,
              height,
              wallPoints,
              wallLines, [],
    ...
    if not gt:
        newIcons, newIconPoints, newDoorLines, newDoorPoints = findCandidatesFromHeatmaps(
            iconHeatmaps, len(iconPoints), len(doorPoints))
    ...
    if True:
        drawLines(os.path.join(debug_prefix, 'lines.png'),
                  ...
                  2,
                  lineColor=255)
        drawLines(os.path.join(debug_prefix, 'doors.png'),
                  width,
                  height,
                  doorPoints,
                  doorLines, [],
                  ...
                  2,
                  lineColor=255)
        drawRectangles(os.path.join(debug_prefix, 'icons.png'), width, height,
                       iconPoints, icons, {}, 2)
        if do_print:
    ...
    doorWallLineMap = findLineMap(doorPoints,
                                  doorLines,
                                  wallPoints,
                                  wallLines,
                                  gap=GAPS['wall_door_neighbor'])
    ...
    conflictWallLinePairs = findConflictLinePairs(
        wallPoints,
        wallLines,
        gap=GAPS['wall_conflict'],
        distanceThreshold=DISTANCES['wall'],
        considerEndPoints=True)

    conflictDoorLinePairs = findConflictLinePairs(
        doorPoints,
        doorLines,
        gap=GAPS['door_conflict'],
        distanceThreshold=DISTANCES['door'])
    conflictIconPairs = findConflictRectanglePairs(iconPoints,
                                                   icons,
                                                   gap=GAPS['icon_conflict'])

    if False:
        print(wallLines)
        os.system('mkdir ' + debug_prefix + '/lines')
        for lineIndex, line in enumerate(wallLines):
            drawLines(os.path.join(debug_prefix,
                                   'lines/line_' + str(lineIndex) + '.png'),
                      width,
                      height,
                      wallPoints, [line], [],
                      lineColor=255)
    ...
    wallLineNeighbors = findLineNeighbors(wallPoints,
                                          wallLines,
                                          labelVotesMap,
                                          gap=GAPS['wall_neighbor'])

    iconWallLineNeighbors = findRectangleLineNeighbors(
        iconPoints,
        icons,
        wallPoints,
        wallLines,
        wallLineNeighbors,
        gap=GAPS['wall_icon_neighbor'],
        distanceThreshold=DISTANCES['wall_icon'])
    conflictIconWallPairs = findConflictRectangleLinePairs(
        iconPoints,
        icons,
        wallPoints,
        wallLines,
        gap=GAPS['wall_icon_conflict'])
    ...
    if False:
        ...
        for wallIndex in [0, 17]:
            ...
            print(
                [wallPoints[pointIndex] for pointIndex in wallLines[wallIndex]])
    ...
    if False:
        ...
        for lineIndex, neighbors in enumerate(wallLineNeighbors):
            if len(neighbors[0]) == 0 and len(neighbors[1]) > 0:
                ...
                pass
            continue
        drawLines(os.path.join(debug_prefix, 'exterior_1.png'),
                  width,
                  height,
                  wallPoints,
                  filteredWallLines,
        ...
        for lineIndex, neighbors in enumerate(wallLineNeighbors):
            if len(neighbors[0]) > 0 and len(neighbors[1]) == 0:
                ...
                pass
            continue
        drawLines(os.path.join(debug_prefix, 'exterior_2.png'),
                  width,
                  height,
                  wallPoints,
                  filteredWallLines,
    ...
    if True:
        ...
        #add variables
        w_p = [
            LpVariable(cat=LpBinary, name="point_" + str(pointIndex))
            for pointIndex in range(len(wallPoints))
        ]
        w_l = [
            LpVariable(cat=LpBinary, name="line_" + str(lineIndex))
            for lineIndex in range(len(wallLines))
        ]

        d_l = [
            LpVariable(cat=LpBinary, name="door_line_" + str(lineIndex))
            for lineIndex in range(len(doorLines))
        ]

        i_r = [
            LpVariable(cat=LpBinary, name="icon_rectangle_" + str(lineIndex))
            for lineIndex in range(len(icons))
        ]

        i_types = []
        for iconIndex in range(len(icons)):
            i_types.append([
                LpVariable(cat=LpBinary,
                           name="icon_type_" + str(iconIndex) + "_" +
                           str(typeIndex)) for typeIndex in range(NUM_ICONS)
            ])
            continue
        ...
        for lineIndex in range(len(wallLines)):
            ...
            for direction in range(2):
                ...
                for label in range(NUM_ROOMS):
                    labels.append(
                        LpVariable(cat=LpBinary,
                                   name="line_" + str(lineIndex) + "_" +
                                   str(direction) + "_" + str(label)))
        ...
        if gt:
            for pointIndex in range(len(wallPoints)):
                model += (w_p[pointIndex] == 1,
                          'gt_point_active_' + str(pointIndex))
                continue
        ...
        for lineIndex in range(len(wallLines)):
            for direction in range(2):
                ...
                for label in range(NUM_ROOMS):
                    ...
                    continue
                model += (labelSum == w_l[lineIndex],
                          'label_sum_' + str(lineIndex) + '_' + str(direction))
                continue
            continue
        ...
        if False:
            ...
            for lineIndex in range(len(wallLines)):
                for oppositeRoomPair in oppositeRoomPairs:
                    model += (l_dir_labels[lineIndex][0][oppositeRoomPair[0]] +
                              l_dir_labels[lineIndex][0][oppositeRoomPair[1]] <=
                              1)
                    if oppositeRoomPair[0] != oppositeRoomPair[1]:
                        model += (
                            l_dir_labels[lineIndex][0][oppositeRoomPair[1]] +
                            l_dir_labels[lineIndex][0][oppositeRoomPair[0]] <=
                            1)
                        pass
                    continue
                continue
        ...
        for label in range(NUM_ROOMS):
            if not closeRooms[label]:
                continue
            for pointIndex, orientationLinesMap in enumerate(
                    wallPointOrientationLinesMap):
                for orientation, lines in orientationLinesMap.items():
                    ...
                    for nextOrientation in range(orientation + 1, 8):
                        ...
                        for nextLineIndex in nextLines:
                            nextLineSum += l_dir_labels[nextLineIndex][
                                nextDirection][label]
                            continue
        ...
        for lineIndex in range(len(wallLines)):
            ...
            label = 0
            model += (l_dir_labels[lineIndex][0][label] +
                      l_dir_labels[lineIndex][1][label] == w_l[lineIndex],
                      'exterior_wall_' + str(lineIndex))
        ...
        for lineIndex, directionNeighbors in enumerate(wallLineNeighbors):
            for direction, neighbors in enumerate(directionNeighbors):
                ...
                for label in range(NUM_ROOMS):
                    obj += (l_dir_labels[lineIndex][direction][label] *
                            (0.0 - labelVotesSum[label]) * labelWeight)
                    continue
                continue
            continue
        ...
        for lineIndex, line in enumerate(wallLines):
            ...
            fixedValue = int(
                round((point_1[1 - lineDim] + point_2[1 - lineDim]) // 2))
            point_1[lineDim], point_2[lineDim] = min(point_1[lineDim],
                                                     point_2[lineDim]), max(
                                                         point_1[lineDim],
                                                         point_2[lineDim])
            ...
            wallLineConfidence = np.sum(
                wallLineConfidenceMap[point_1[1]:point_2[1] +
                                      1, point_1[0]:point_2[0] + 1]) / (
                                          (point_2[1] + 1 - point_1[1]) *
                                          (point_2[0] + 1 - point_1[0])) - 0.5
        ...
        for lineIndex, line in enumerate(doorLines):
            ...
            if not gt:
                doorLineConfidence = np.sum(
                    doorLineConfidenceMap[point_1[1]:point_2[1] +
                                          1, point_1[0]:point_2[0] + 1]) / (
                                              (point_2[1] + 1 - point_1[1]) *
                                              (point_2[0] + 1 - point_1[0]))

                if lineDim == 0:
                    doorPointConfidence = (
                        doorCornerHeatmaps[point_1[1], point_1[0], 3] +
                        doorCornerHeatmaps[point_2[1], point_2[0], 1]) / 2
                else:
                    doorPointConfidence = (
                        doorCornerHeatmaps[point_1[1], point_1[0], 0] +
                        doorCornerHeatmaps[point_2[1], point_2[0], 2]) / 2
                    pass
                doorConfidence = (doorLineConfidence +
                                  doorPointConfidence) * 0.5 - 0.5
        ...
        for iconIndex, icon in enumerate(icons):
            ...
            iconTypeConfidence = iconHeatmaps[y_1:y_2 + 1, x_1:x_2 +
                                              1, :NUM_ICONS +
                                              1].sum(axis=(0, 1)) / iconArea
            iconTypeConfidence = iconTypeConfidence[1:] - iconTypeConfidence[0]

            if not gt:
                iconPointConfidence = (
                    iconCornerHeatmaps[int(round(point_1[1])),
                                       int(round(point_1[0])), 2] +
                    iconCornerHeatmaps[int(round(point_2[1])),
                                       int(round(point_2[0])), 3] +
                    iconCornerHeatmaps[int(round(point_3[1])),
                                       int(round(point_3[0])), 1] +
                    iconCornerHeatmaps[int(round(point_4[1])),
                                       int(round(point_4[0])), 0]) // 4 - 0.5
                iconConfidence = (iconTypeConfidence +
                                  iconPointConfidence) * 0.5
            else:
                iconConfidence = iconTypeConfidence
                pass
            ...
            for typeIndex in range(NUM_ICONS):
                obj += (-i_types[iconIndex][typeIndex] *
                        (iconConfidence[typeIndex]) * iconTypeWeight)
                continue
            continue
        ...
        for pointIndex, orientationLinesMap in enumerate(
            ...
            for orientation, lines in orientationLinesMap.items():
                ...
                model += (lineSum == w_p[pointIndex], "line_sum_" +
                          str(pointIndex) + "_" + str(orientation))
                continue
            continue

        ## Conflict constraints
        for index, conflictLinePair in enumerate(conflictWallLinePairs):
            model += (w_l[conflictLinePair[0]] + w_l[conflictLinePair[1]] <= 1,
                      'conflict_wall_line_pair_' + str(index))
            continue

        for index, conflictLinePair in enumerate(conflictDoorLinePairs):
            model += (d_l[conflictLinePair[0]] + d_l[conflictLinePair[1]] <= 1,
                      'conflict_door_line_pair_' + str(index))
            continue

        for index, conflictIconPair in enumerate(conflictIconPairs):
            model += (i_r[conflictIconPair[0]] + i_r[conflictIconPair[1]] <= 1,
                      'conflict_icon_pair_' + str(index))
            continue

        for index, conflictLinePair in enumerate(conflictIconWallPairs):
            model += (i_r[conflictLinePair[0]] + w_l[conflictLinePair[1]] <= 1,
                      'conflict_icon_wall_pair_' + str(index))
            continue

        ## Door wall constraints (a door must sit on one and only one wall)
        for doorIndex, lines in enumerate(doorWallLineMap):
            if len(lines) == 0:
                model += (d_l[doorIndex] == 0,
                          'door_not_on_walls_' + str(doorIndex))
                continue
            lineSum = LpAffineExpression()
            for lineIndex in lines:
                lineSum += w_l[lineIndex]
                continue
            model += (d_l[doorIndex] <= lineSum,
                      'd_wall_line_sum_' + str(doorIndex))
            continue

        doorWallPointMap = findLinePointMap(doorPoints,
                                            doorLines,
                                            wallPoints,
                                            gap=GAPS['door_point_conflict'])
        for doorIndex, points in enumerate(doorWallPointMap):
            if len(points) == 0:
                continue
            pointSum = LpAffineExpression()
            for pointIndex in points:
                model += (d_l[doorIndex] + w_p[pointIndex] <= 1,
                          'door_on_two_walls_' + str(doorIndex) + '_' +
                          str(pointIndex))
                continue
            continue
        ...
        if do_print:
            print('Optimization information', LpStatus[model.status],
        ...
        if LpStatus[model.status] == 'Optimal':
            ...
            filteredWallTypes = []
            wallPointLabels = [
                [-1, -1, -1, -1] for pointIndex in range(len(wallPoints))
            ]
            ...
            for lineIndex, lineVar in enumerate(w_l):
                ...
                for direction in range(2):
                    for label in range(NUM_ROOMS):
                        if l_dir_labels[lineIndex][direction][
                                label].varValue > 0.5:
                ...
                if do_print:
                    print('wall', lineIndex, labels, [
                        np.array(wallPoints[pointIndex][:2]).astype(
                            np.int32).tolist()
                        for pointIndex in wallLines[lineIndex]
                    ], wallLineNeighbors[lineIndex][0].keys(),
                          wallLineNeighbors[lineIndex][1].keys())
            ...
            if not gt:
                ...
                adjustPoints(wallPoints, filteredWallLines)
                filteredWallLabels = [
                    filteredWallLabels[lineIndex]
                    for lineIndex in range(len(filteredWallLines))
                    if filteredWallLines[lineIndex][0] !=
                    filteredWallLines[lineIndex][1]
                ]
                filteredWallLines = [
            ...
            drawLines(output_prefix + 'result_line.png',
                      width,
                      height,
                      wallPoints,
                      filteredWallLines,
            ...
            for lineIndex, lineVar in enumerate(d_l):
                ...
                if do_print:
                    print(('door', lineIndex, [
                        doorPoints[pointIndex][:2]
                        for pointIndex in doorLines[lineIndex]
                    ]))
            ...
            filteredDoorWallMap = findLineMapSingle(
                doorPoints,
                filteredDoorLines,
                wallPoints,
                filteredWallLines,
                gap=GAPS['wall_door_neighbor'])
            adjustDoorPoints(doorPoints, filteredDoorLines, wallPoints,
                             filteredWallLines, filteredDoorWallMap)
            drawLines(output_prefix + 'result_door.png',
                      width,
                      height,
                      doorPoints,
                      filteredDoorLines,
            ...
            for iconIndex, iconVar in enumerate(i_r):
                ...
                if do_print:
                    print(('icon', iconIndex, iconType, [
                        iconPoints[pointIndex][:2]
                        for pointIndex in icons[iconIndex]
                    ]))
            ...
            drawRectangles(output_prefix + 'result_icon.png', width, height,
                           iconPoints, filteredIcons, filteredIconTypes)
            ...
            orientationMap = {}
            for pointType, orientationOrientations in enumerate(
                    POINT_ORIENTATIONS):
                for orientation, orientations in enumerate(
            ...
            for pointIndex, point in enumerate(wallPoints):
                orientations = []
                orientationLines = {}
                for orientation, lines in wallPointOrientationLinesMap[
                        pointIndex].items():
                    orientationLine = -1
                    for lineIndex in lines:
                ...
                if len(orientations) < len(
                        wallPointOrientationLinesMap[pointIndex]):
                    if do_print:
                        print('invalid point', pointIndex, orientations,
                              wallPointOrientationLinesMap[pointIndex])
            ...
            with open(output_prefix + 'floorplan.txt', 'w') as result_file:
                ...
                for wallIndex, wall in enumerate(filteredWallLines):
                    ...
                    result_file.write(
                        str(filteredWallLabels[wallIndex][0]) + '\t' +
                        str(filteredWallLabels[wallIndex][1]) + '\n')
                ...
                for doorIndex, door in enumerate(filteredDoorLines):
                    ...
                    result_file.write(
                        str(filteredDoorTypes[doorIndex] + 1) + '\t1\n')
                ...
                for iconIndex, icon in enumerate(filteredIcons):
                    ...
                    result_file.write(
                        iconNumberNameMap[filteredIconTypes[iconIndex]] + '\t')
    ...
    result_dict = {
        'wall': [wallPoints, filteredWallLines, filteredWallLabels],
        'door': [doorPoints, filteredDoorLines, []],
        'icon': [iconPoints, filteredIcons, filteredIconTypes]
    }
    return result_dict
